...
export const QuotationForm = ({ initialData }: QuotationFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        quotationDate,
        expiryDate: expiryDate || quotationDate,
        notes,
        subtotal,
        taxTotal,
        total,
      ...
      if (initialData?.id) {
        await api.patch(`/quotations/${initialData.id}`, payload);
      ...
      toast.error(error.response?.data?.message || 'An error occurred during quotation saving.');
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-8 bg-white rounded-3xl border border-gray-100 shadow-sm">
        ...
        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-700">Quotation Date</label>
          <input 
            type="date"
            value={quotationDate}
